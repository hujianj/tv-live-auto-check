...
def write_report(report: dict[str, Any]) -> None:
    ...
    if report.get("status") in {"ok", "failed"}:
        try:
            from export_outputs import export_outputs
            export_outputs(ROOT, report)
        except Exception as exc:
            print(f"MAINTENANCE WARN: diagnostic export failed: {exc}", flush=True)
...
def cleanup_stale_run_outputs() -> None:
    ...
    for name in STALE_RUN_OUTPUTS:
        ...
        if not path.resolve().is_relative_to(ROOT.resolve()):
            raise RuntimeError(f"stale output escapes maintenance root: {name}")
        if not path.is_file():
            continue
        try:
            path.unlink()
...
def main(argv: list[str] | None = None) -> int:
    ...
    env = os.environ.copy()
    env["IPTV_SCAN_RUN_ID"] = uuid.uuid4().hex
