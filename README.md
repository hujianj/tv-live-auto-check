...
`verify_sources.py` 和 `recheck_published.py` 两个真实网络检测阶段允许有限重试；重试从失败阶段继续，不再重新跑已经成功的全量上游检测。防缩水守卫如果第一次因网络波动或上游瞬时缩水拒绝，会自动从 `verify_sources.py` 重新跑一轮独立确认，确认轮仍失败才停止发布；两轮证据会作为小型 artifact 保存。分类、质量审计、防缩水、体积和清单校验属于确定性阶段，失败会立即停止发布并在告警 Issue 中写明失败脚本、退出码、分类和耗时。排队期间如果 `main` 已更新，旧运行会在昂贵检测前自动跳过；新的代码 push 会取消正在使用旧代码检测的运行，但计划和手动运行只排队、不互相取消。
