...
Actions 的运行摘要会列出每阶段耗时、是否超时、复检中止原因及实际通过的候选数。`Code and configuration` 与 `Existing publication integrity` 为独立检查；旧发布文件审计失败仍会显示红色，不会通过修改统计或忽略错误隐藏。只有媒体检测、覆盖、质量和发布文件校验全部通过才更新订阅，代码部署成功不等于节目更新成功。
