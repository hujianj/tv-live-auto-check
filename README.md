...
频道覆盖目标是 CCTV-1 至 CCTV-17、CCTV-5+ 以及各省常见中文卫视，少儿、影视、娱乐作为补充。`coverage-report.md` 单独显示覆盖是否完整和缺台名单；Actions 成功只表示本轮真实检测及发布完成，不表示频道已经齐全。已修复英文名称的国内台被误作外语排除，以及 CCTV-9/纪录等官方别名重复计数的问题。

上游 [vbskycn/iptv](https://github.com/vbskycn/iptv) 提供的 [live.zbds.top](https://live.zbds.top/) 已于 2026-09-09 补齐明确引用说明和 GPL-3.0 依据，恢复四种格式的媒体候选检测，IPv4 TXT 保持最高来源优先级。派生链接选集遵循 [GPL-3.0](docs/GPL-3.0.txt)，原始出处、筛选修改及边界见 [来源记录](docs/source-permissions.md)；许可证不覆盖电视节目内容。
