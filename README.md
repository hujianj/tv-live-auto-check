...
2026-09-08 核查：云端第 177 次主维护在首检阶段两次达到 1800 秒后退出，第二次重新检测全部 22849 个 URL。修复版第 181 次运行在约 65 秒内检查完筛选范围内全部 243 个 URL，123 项测试通过；严格复检仍有 10 个地址失败，超过未修改的 25% 门槛，未发布。原因包含清单停止推进、404 和解码失败，不能解释成 CDN 问题。代码修复与新订阅发布分开处理，旧订阅文件不能作为本轮播放验证通过的证据。完整记录见 [迁移审查记录](docs/review-2026-09-07.md)。
...
Actions 的运行摘要会列出每阶段耗时、是否超时、复检中止原因及实际通过的候选数。`Code and configuration` 与 `Existing publication integrity` 为独立检查；旧发布文件审计失败仍会显示红色，不会通过修改统计或忽略错误隐藏。依赖安装、FFmpeg 预检和网络安装均有独立进程时限，卡住时会明确失败而不会占满整轮。只有媒体检测、覆盖、质量和发布文件校验全部通过才更新订阅，代码部署成功不等于节目更新成功。
